...
- **CI не изменяет dependency graph проекта.** Основные зависимости всегда устанавливаются через `npm ci`. Vitest 4.1.10 в GitHub Actions запускается из отдельного временного prefix, чтобы установка test runner не модифицировала `node_modules`, `package-lock.json` или разрешение production-зависимостей проекта.
