-- TTV immersion lab: keep `hit_points` canonical in storage, but return a
-- transient `hp` alias from the quick-HP RPC so legacy in-memory VTT widgets do
-- not lose the health resource between the mutation and the next refresh.
...
begin
  ...
  if health_key = 'hit_points' then
    return jsonb_set(current_data, '{hp}', current_data->'hit_points', true);
  end if;
  return current_data;
end;
$$;
